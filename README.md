...
Each version entry defines a `name`, a `version`, and a list of at least two `entries` containing a file `path` and a regex `pattern` with a single capture group for the version.
